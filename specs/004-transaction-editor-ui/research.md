...
- **Correction found during implementation**: this decision's original premise — "TxnDate is already normalised" — is not universally true. `NormaliseDate` falls back to returning its input unchanged when no known layout matches, and the real `data/gmail/transactions.csv` has a small number of legacy rows where that happened (e.g. `"Jul 22, 2024 05:46 PM"`, `"17/07/XXXX"`). A raw string comparison sorted one of these — a transaction from 2024 — ahead of every transaction from 2026, breaking FR-002/SC-001. Fixed by an `isNormalisedDate` guard (`webui/server.go`) that checks the `YYYY-MM-DD` prefix shape before trusting the comparison; anything that fails the check is treated like an empty date (sorted last), and the same guard now gates the `from`/`to` date-range filter for the same reason.
